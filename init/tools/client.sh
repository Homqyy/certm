...
#
# tassl  ca -config sub-ca.conf -revoke certs/682852BD7857319DD3FDBE34C10C838D.pem -crl_reason "keyCompromise"
#
...
function gen_rsa
{
    ...
    $g_openssl ca -config sub-ca.conf -days 1095 -in $cert_dir/priv.csr -out $cert_dir/cert.pem -extensions client_ext -notext -passin pass:$conf_passwd
...
function gen_gm
{
    ...
    $g_openssl ca -config gm-sub-ca.conf -days 1095 -in $cert_dir/priv.csr -out $cert_dir/cert.pem -extensions client_gm_ext -md sm3 -notext -passin pass:$conf_passwd
    [ $? -ne 0 ] && cd - && exit_on_error 1
    ...
    $g_openssl ca -config gm-sub-ca.conf -days 1095 -in $cert_dir/enc-priv.csr -out $cert_dir/enc-cert.pem -extensions client_gm_enc_ext -md sm3 -notext -passin pass:$conf_passwd
    [ $? -ne 0 ] && cd - && exit_on_error 1
